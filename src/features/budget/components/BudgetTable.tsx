...
import { TransactionsDrawer, DrawerListingTarget } from '@/features/statistics/components/DistributionDonut/TransactionsDrawer';
...
const BudgetTable: React.FC<Props> = ({ budgetId, budget, analytics, displayCurrency, rates }) => {
        ...
        for (const [currency, cv] of Object.entries(item.convertedValues)) {
          const rate =
            currency === displayCurrency
              ? 1
              : getExchangeRate(currency, displayCurrency, rates);
          if (rate !== null) {
  ...
  const expenseRoots = sortCats(catData?.tree.filter((c) => c.isAffectingProfit && c.type === CategoryType.Expense) ?? []);
  const incomeRoots = sortCats(catData?.tree.filter((c) => c.isAffectingProfit && c.type === CategoryType.Income) ?? []);
    ...
    return (
      <tr className="bg-muted/40 font-semibold text-sm border-t-2">
        ...
        <td className="py-2 px-2 text-right tabular-nums">
          {planned > 0 ? fmtAmt(planned, displayCurrency) : '—'}
        </td>
        <td className="py-2 px-2 text-right tabular-nums">
          {actual > 0 ? fmtAmt(actual, displayCurrency) : '—'}
        </td>
        <td className={`py-2 px-4 text-right tabular-nums ${remaining < 0 ? 'text-destructive' : 'text-green-600 dark:text-green-400'}`}>
          {planned > 0 ? (
            <>
    ...
    <thead>
      <tr className="border-b bg-muted/50 text-xs text-muted-foreground">
        ...
        <th className="py-2 px-2 text-right font-medium whitespace-nowrap">
          Planned ({displayCurrency})
        </th>
        <th className="py-2 px-2 text-right font-medium whitespace-nowrap">
          Actual ({displayCurrency})
        </th>
