...
import { BankProvider, useBankIntegrationList, useSyncBankIntegration } from '@/features/bank-integrations';
...
const BankSheet: React.FC<Props> = ({ account, onAccountUpdate }) => {
  ...
  const providerRaw = integration
    ? (typeof integration.provider === 'string' ? integration.provider : (integration.provider as any)?.value ?? '')
    : '';
    ...
    if (hasAnyIntegration) {
      return (
        <div className="flex flex-col items-center justify-center gap-4 py-8 text-center">
          ...
          <Button variant="outline" onClick={() => { setOpen(false); setConnectDialogOpen(true); }}>
    ...
    <>
      <Tooltip>
        <TooltipTrigger asChild>
          <Button aria-label="Bank connection" size="icon" variant="outline" className="relative" onClick={() => setOpen(true)}>
            <span className="sr-only">Manage bank connection</span>
            <Building2 aria-hidden="true" className="h-4 w-4" />
            <span className={cn('absolute -top-0.5 -right-0.5 h-2 w-2 rounded-full border border-background', statusColor)} />
          </Button>
        </TooltipTrigger>
