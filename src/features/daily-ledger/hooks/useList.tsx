...
export const useTransactionsAndTransfersList = ({
  ...
  const queryParams: LedgerQueryParams = useMemo(
    () => ({
      ...
      before: transactionFilters.before,
      type: backendType,
      accounts: (transactionFilters.accounts as number[] | undefined) ?? undefined,
      categories: (transactionFilters.categories as number[] | undefined) ?? undefined,
      debts: (transactionFilters.debts as number[] | undefined) ?? undefined,
  ...
  // gcTime: 0 → cache cleared immediately on unmount → no stale data when
  // navigating between pages that share the same filter defaults.
  ...
  useFormSubmitListener([FormType.Transaction, FormType.Transfer], () => {
    ...
    // Also invalidate standalone caches so those pages show fresh data when navigated to
  ...
  // Only the fields actually accessed externally are provided.
