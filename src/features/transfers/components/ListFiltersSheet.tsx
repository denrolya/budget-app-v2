...
const Content: React.FC<Omit<Props, 'isOpen' | 'setIsOpen'>> = ({ data, onChange, onReset }) => {
  const [minLocal, setMinLocal] = useState('');
  const [maxLocal, setMaxLocal] = useState('');
  const [searchLocal, setSearchLocal] = useState((data as any).searchTerm ?? '');
  ...
  const debouncedSearch = useRef(
    debounce((value: string) => {
      onChange('searchTerm' as any, value as any);
    }, 250),
  ).current;
    ...
    <FilterWrapper open={isOpen} onOpenChange={setIsOpen}>
      <FilterContent side={isMobile ? undefined : 'right'} className={isMobile ? undefined : 'w-96'}>
        <FilterHeader>
          ...
          <FilterDescription className="sr-only">Additional transfer filter options.</FilterDescription>
        </FilterHeader>
        <div className="overflow-y-auto px-4 pb-6 pt-2">
          <Content data={data} onChange={onChange} onReset={onReset} />
        </div>
      </FilterContent>
    </FilterWrapper>
  );
