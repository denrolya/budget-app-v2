...
const KeyboardKey: React.FC<{ children: React.ReactNode }> = ({ children }) => (
  <span className="inline-flex items-center justify-center min-w-[2em] h-8 px-2 text-xs font-semibold text-secondary-foreground bg-secondary rounded-md border border-secondary-foreground/20 shadow-[inset_0_-2px_0_0_hsl(var(--foreground)/0.2),0_1px_2px_0_hsl(var(--foreground)/0.1)] transition-all duration-100 ease-in-out hover:translate-y-[1px] hover:shadow-[inset_0_-1px_0_0_hsl(var(--foreground)/0.2),0_1px_1px_0_hsl(var(--foreground)/0.1)] active:translate-y-[2px] active:shadow-none mx-0.5">
    {children}
  </span>
);
