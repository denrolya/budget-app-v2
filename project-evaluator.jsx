...
function ProjectCard({ project, onUpdate, onRemove, index }) {
  ...
  const costMultiplier = project.cost === 'low' ? 1 : project.cost === 'medium' ? 0.7 : 0.4;
            ...
            {URGENCY_OPTIONS.map((opt) => (
              <button
                ...
                style={{
                  padding: '5px 10px',
                  borderRadius: 6,
                  border: project.urgency === opt.value ? `1px solid ${opt.color}` : '1px solid #1a1a2e',
                  background: project.urgency === opt.value ? `${opt.color}18` : '#0d0d1a',
            ...
            {COST_OPTIONS.map((opt) => (
              <button
                ...
                style={{
                  padding: '5px 10px',
                  borderRadius: 6,
                  border: project.cost === opt.value ? `1px solid ${opt.color}` : '1px solid #1a1a2e',
                  background: project.cost === opt.value ? `${opt.color}18` : '#0d0d1a',
...
function SummaryTable({ projects }) {
  const evaluated = projects.filter((p) => CRITERIA.every((c) => p.scores[c.id] !== null));
...
function generateMarkdown(projects) {
  ...
  sorted.forEach((p, i) => {
    const total = CRITERIA.reduce((s, c) => s + p.scores[c.id], 0);
    const costLabel = p.cost === 'low' ? '< 2ч' : p.cost === 'medium' ? '2-5ч' : '> 5ч';
    const costM = p.cost === 'low' ? 1 : p.cost === 'medium' ? 0.7 : 0.4;
    const eff = (total * costM).toFixed(1);
    const urgLabel =
      p.urgency === 'high' ? 'Месяц' : p.urgency === 'medium' ? 'Сезон' : p.urgency === 'low' ? 'Год' : '—';
